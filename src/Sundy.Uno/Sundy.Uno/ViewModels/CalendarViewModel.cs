...
/// <summary>
/// Main calendar view ViewModel with Month/Week/Day modes.
...
public partial class CalendarViewModel : ObservableObject
{
    ...
    partial void OnSelectedDateChanged(DateTime value) => _ = RefreshViewAsync();
    ...
    private async Task LoadMonthViewAsync()
    {
        ...
        var events = await _mediator.Send(new GetEventsInRangeQuery(
            startRange, endRange, SelectedCalendar?.Id, VisibleCalendarIds)).ConfigureAwait(false);
        ...
        for (var date = startDate; date < endDate; date = date.AddDays(1))
        {
            ...
            var gridRow = index / 7;
            var gridColumn = index % 7;

            days.Add(new MonthDayViewModel(date, isCurrentMonth, isToday, dayEvents, gridRow, gridColumn, OnMonthDayEventSelected));
            index++;
        }
    ...
    {
        await _mediator.Send(new UpdateEventCommand(updatedEvent), CancellationToken.None).ConfigureAwait(false);
        _ = RefreshViewAsync().ConfigureAwait(false);
    }
    ...
    [RelayCommand]
    private void RequestNewEventForDate(DateTime date) => NewEventForDateRequested?.Invoke(this, date);

    /// <summary>
    /// Public method to request event editing - called from CalendarView.
    /// </summary>
    public void RequestEditEvent(EventViewModel eventVm)
    {
        if (eventVm.Event != null)
        {
            EventEditRequested?.Invoke(this, eventVm.Event);
        }
    }

    private void OnEventEditRequested(object? sender, CalendarEvent evt) => EventEditRequested?.Invoke(this, evt);
