...
struct DayView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            DayHeaderView(date: currentDate)
...
private struct DayHeaderView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            VStack(spacing: 4) {
                Text(dayName(date).uppercased())
                    .font(ThemeTypography.body(11))
                    .foregroundStyle(ThemeColors.textMuted.swiftUIColor)
                Text("\(Calendar.current.component(.day, from: date))")
                    .font(ThemeTypography.body(32))
                    .foregroundStyle(ThemeColors.textSecondary.swiftUIColor)
                    .padding(6)
                    .background(
                        Calendar.current.isDateInToday(date) ? Circle().fill(ThemeColors.accent.swiftUIColor) : Circle().fill(Color.clear)
                    )
            }
            .frame(maxWidth: .infinity)
            .padding(.vertical, 8)
            .background(Calendar.current.isDateInToday(date) ? ThemeColors.accent.swiftUIColor.opacity(0.1) : Color.clear)
        }
